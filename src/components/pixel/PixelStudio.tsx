...
export default function PixelStudio() {
  ...
  const render = useCallback(() => {
    ...
    if (onionMode > 0) {
      ...
      const drawTinted = (f: Frame, color: string) => {
        const c = document.createElement("canvas");
      ...
      if (prev) drawTinted(prev, "#ff2222"); // Red tint for past
      if (next) drawTinted(next, "#22ff22"); // Green tint for future
      ctx.globalAlpha = 1;
    }
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (tool === "brush" || tool === "eraser" || tool === "dither") {
      ...
      // keep accumulation: also merge previous work
      const prev = baseRef.current!;
      // brush should accumulate on base+trail; simplest: keep workRef persistent
      lastRef.current = [x, y];
      // accumulate: apply on the persistent workRef by reusing it
      workRef.current = accumulateBrush(prev, work);
    } else if (tool === "line") {
      stampPoints(work, linePoints(start[0], start[1], x, y), fillColor);
    } else if (tool === "rect") {
      stampPoints(work, rectPoints(start[0], start[1], x, y, false), fillColor);
    } else if (tool === "circle") {
      stampPoints(work, ellipsePoints(start[0], start[1], x, y, false), fillColor);
    } else if (tool === "move") {
      const dx = x - start[0],
        dy = y - start[1];
          ...
          >
            {TOOLS.map((t) => (
              <Tooltip key={t.id}>
                ...
                <TooltipContent side="right" className="flex items-center gap-2 rounded-lg border-white/10 bg-card/95 backdrop-blur-md">
